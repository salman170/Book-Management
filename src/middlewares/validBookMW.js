...
const createBookMW = async function (req, res, next) {
    try {
        ...
        let data = req.body;
        if (Object.keys(data).length == 0) return res.status(400).send({ status: false, msg: "please enter require data to create Book" })
        ...
        if (typeof (title) !== "string") return res.status(400).send({ status: false, msg: "Title will be in string format only" })


        if (!excerpt) return res.status(400).send({ status: false, msg: "Excerpt is mandatory" })

        if (!/^[a-zA-Z 0-9.,''-\s]+$/.test(excerpt)) return res.status(400).send({ status: false, msg: "Please Enter  Alphabets and ( 0-9 .,-'' )in excerpt" })
        ...
        if (!category || typeof category != "string" || !(category.trim())) return res.status(400).send({ status: false, msg: "Category is mandatory & should be string" })

        if (!/^[a-zA-Z \s]+$/.test(category)) return res.status(400).send({ status: false, msg: "Please Enter Only Alphabets in Category" })

        if (!subcategory || typeof subcategory != "string" || !(subcategory.trim())) return res.status(400).send({ status: false, msg: "Subcategory is mandatory & should be string" })

        if (!/^[a-zA-Z \s]+$/.test(subcategory)) return res.status(400).send({ status: false, msg: "Please Enter Only Alphabets in subcategory" })

        if (reviews) {
            if (typeof (reviews) !== "number" && reviews != 0) return res.status(400).send({ status: false, msg: "Reviews will be in number format only and should be 0 while creating book" })
        }
